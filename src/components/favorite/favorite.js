...
function Favorite() {
  ...
  const {photo,setPhoto} = useContext(OpenPhotoContext)
  function removeFavPhoto(obj){
    console.log(obj.photo.urls.full)
  }
  if (favPhoto == null){
    return null
  } 
  const favPhotos = favPhoto.map((favPhoto)=>(
    <div className="grid-item" key = {favPhoto.id}>
            <div className="photos-block-background">
                ...
                <span>{favPhoto.photo.user.first_name}</span>
            </div>  
            <div className='photos-block-content'> 
                ...
                <div className="content-icons">
                  <div className="content-icons-like content-icons-item">
                    <button onClick={() => removeFavPhoto(favPhoto)}>dislike</button>
                  </div>
                  <div className="content-icons-open content-icons-item">
                    
                    <Link to="/open-photo" onClick={() => isOpen(favPhoto)}>open</Link>
                  </div>
